...
Friend Class F_TMP_PTNCODE2
	...
	Private Sub Command1_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Command1.Click
		...
		'/ 入力チェック /
		If check_F_TMP_PTNCODE2 <> 0 Then Exit Sub
		
		form_no.Enabled = False
		F_MSG.Show()
		
		'(Brand Ver.3 追加)
        For i = 1 To Len(form_no.w_ptncode.Text)
            ...
            If IsNumeric(w_str) Then
                If Val(w_str) >= 0 And Val(w_str) < 10 Then
                    If GensiNUM(Val(w_str)) = "" Then
                        MsgBox("A substituted primitive letter for input Pattern code is not set to the configuration file (" & Tmp_Pattern2_ini & ")", 64, "Configuration file error")
                        GoTo error_section
                    End If
                End If
            ElseIf w_str = "+" Then
        ...
        temp_bz.pattern = LSet(Trim(form_no.w_ptncode.Text), Len(temp_bz.pattern))
		...
		'// 終了の送信
        w_mess = Tmp_Pattern2_ini
        w_ret = PokeACAD("TMPNAME", w_mess)
		For i = 1 To Tmp_font_cnt + 1
            If Tmp_font_word(i) = form_no.w_font.Text Then
                w_mess = "TYPE" & i
                w_ret = PokeACAD("TMPDATANO", w_mess)
                Exit For
            End If
		Next i
        w_mess = Trim(form_no.w_ptncode.Text)
...
error_section: 
	...
	Private Sub F_TMP_PTNCODE2_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        ret = set_read4(w_w_str, "pattern2", 1)
	...
	Private Sub w_font_SelectedIndexChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles w_font.SelectedIndexChanged
		...
		For i = 1 To Tmp_font_cnt + 1
			If Tmp_font_word(i) = w_font.Text Then
				w_w_str = Environ("ACAD_SET")
                w_w_str = Trim(w_w_str) & Trim(Tmp_Pattern2_ini)
                ret = set_read4(w_w_str, "pattern2", i)
                If ret = False Then
                    MsgBox(Tmp_Pattern2_ini & "File reading error.", 64, "BrandVB error")
                    Exit Sub
                Else
                    read_flg = 1
                    Exit For
		...
		If read_flg = 0 Then
            MsgBox("Font type of data that are selected, not set configuration file (" & Tmp_Pattern2_ini & ")", 64, "Configuration file error")
			Exit Sub
		End If
