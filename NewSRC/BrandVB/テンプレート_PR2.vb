...
Friend Class F_TMP_PR2
	...
	Private Sub Command1_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Command1.Click
		...
		Dim i As Short
		Dim pr_data As String
		
		If check_F_TMP_PR2 <> 0 Then Exit Sub
		
		pr_data = Trim(w_ply1.Text) & Trim(w_ply2.Text)
		
		form_no.Enabled = False
		F_MSG.Show()
		
		'(Brand Ver.3 追加)
		For i = 1 To Len(pr_data)
            w_str = Mid(pr_data, i, 1)
			If IsNumeric(w_str) Then
                If Val(w_str) >= 0 And Val(w_str) < 10 Then
                    If GensiNUM(Val(w_str)) = "" Then
                        MsgBox("A substituted primitive letter for input ply is not set to the configuration file (" & Tmp_Pr2_ini & ")", 64, "Configuration file error")
                        GoTo error_section
                    End If
                End If
            ElseIf w_str = "." Then
		...
		For i = 1 To Len(pr_data)
			...
			If IsNumeric(w_str) Then
                If Val(w_str) >= 0 And Val(w_str) < 10 Then
                    ...
                    If pic_no < 1 Then
                        'UPGRADE_WARNING: オブジェクト gm_no の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        ...
        form_no.w_ply1.Enabled = False
        form_no.w_ply1.BackColor = System.Drawing.ColorTranslator.FromOle(&HC0C0C0) '20100629コード変更
...
error_section: 
	...
	Private Sub F_TMP_PR2_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		Dim w_ret As Object
		Dim ret As Object
		Dim i As Object
	...
	Private Sub w_font_SelectedIndexChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles w_font.SelectedIndexChanged
		...
		For i = 1 To Tmp_font_cnt + 1
			If Tmp_font_word(i) = w_font.Text Then
				w_w_str = Environ("ACAD_SET")
				w_w_str = Trim(w_w_str) & Trim(Tmp_Pr2_ini)
				ret = set_read4(w_w_str, "pr2", i)
				If ret = False Then
                    MsgBox(Tmp_Pr2_ini & "File reading error.", 64, "BrandVB error")
					Exit Sub
				Else
					read_flg = 1
					Exit For
		...
		If read_flg = 0 Then
            MsgBox("Font type of data that are selected, not set configuration file (" & Tmp_Pr2_ini & ")", 64, "Configuration file error")
			Exit Sub
		End If
