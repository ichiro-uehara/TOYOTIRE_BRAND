...
error_section:
	...
	Private Sub Command6_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Command6.Click
            ...
            If IsNumeric(w_str) Then
                If Val(w_str) >= 0 And Val(w_str) < 10 Then
                    If GensiNUM(Val(w_str)) = "" Then
                        MsgBox("Replacement for the primitive character size code has not been set in the configuration file (" & Tmp_Serial2_ini & ")", 64, "Configuration file error")
                        GoTo error_section
                    End If
                End If
        ...
        'w_mess = VB6.Format(Val(pic_no), "00") & HensyuDir & ZumenName
        ...
        For i = 1 To Len(form_no.w_size_code.Text)
            If Mid(form_no.w_size_code.Text, i, 1) >= "0" And Mid(form_no.w_size_code.Text, i, 1) <= "9" Then
                ...
                ' -> watanabe edit 2013.05.29
                'w_mess = VB6.Format(Val(pic_no), "00") & GensiDir & ZumenName
...
error_section:
	...
	Private Sub F_TMP_SERIARU2_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		Dim ret As Object
        ...
        w_w_str = Trim(w_w_str) & Trim(Tmp_Serial2_ini)
        ret = set_read5(w_w_str, "serial2", 1)
		w_plant.Items.Clear()
	...
	Private Sub w_font_SelectedIndexChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles w_font.SelectedIndexChanged
		...
		For i = 1 To Tmp_font_cnt + 1
			If Tmp_font_word(i) = w_font.Text Then
				w_w_str = Environ("ACAD_SET")
                w_w_str = Trim(w_w_str) & Trim(Tmp_Serial2_ini)
                ret = set_read5(w_w_str, "serial2", i)
                If ret = False Then
                    MsgBox(Tmp_Serial2_ini & "File reading error.", 64, "BrandVB error")
                    Exit Sub
                Else
                    read_flg = 1
                    Exit For
		...
		If read_flg = 0 Then
            MsgBox("Font type of data that are selected, not set configuration file (" & Tmp_Serial2_ini & ")", 64, "Configuration file error")
			Exit Sub
		End If
